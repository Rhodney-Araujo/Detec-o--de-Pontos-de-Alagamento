...
def dados_persistentes_false(supabase):
    ...
    if ultimo_id_inserido is not None:
        ...
        supabase.table('sensor').update(dados).eq('Código', ultimo_id_inserido).execute()
        print(f"Alagamento persistente atualizado para False no registro {ultimo_id_inserido} em:", data_hora_atual)
        ultimo_id_inserido = None
    else:
        print("Nenhum ID de registro para atualizar.")
...
while True:
    if arduino.inWaiting() > 0:
        ...
        # Verificação inicial com 10 leituras
        if len(valores_sensor) == 55: #serão 55 verificações
            ...
            if positivos > negativos:
                ...
                valores_sensor = []
                arduino.close()
                time.sleep(600) #serão 10 minutos
